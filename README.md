...
- 当前为有限规则语法，推荐用逗号分句、阿拉伯数字预算，例如“推荐双肩包，预算600元，必须电脑隔层尺寸至少16英寸”。中文数词、别名、复杂口语可能进入追问。
- 品类和明确预算／数值界限用于硬过滤；一般品牌、功能、用途、文本参数未标强度时默认软偏好。必须满足的条件请明确说“必须”，并核对界面的结构化条件。
- “颜色黑色”与“必须颜色黑色”不是同一个强度。当前尚不能可靠推断用户未明说的硬软边界；详见[最终失败案例](docs/evaluation.md#失败案例与下一轮方向)。
...
80条代理编写的合成样本，开发40、最终40，不是独立人工标注集；最终集首次运行后未据此调业务代码。结果对应生产提交 `e4ea227`，完整定义见[评测文档](docs/evaluation.md)。
